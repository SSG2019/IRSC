...
class Transceiver(nn.Module):
    ...
    def forward(self, input_mnist, input_cifar, snr):
        ...
        txm = self.transmitter_mnist(input_mnist)
        txc = self.transmitter_cifar(input_cifar)
        ...
        zm = self.receiver_mnsit(rxm)
        zc = self.receiver_cifar(rxc)
        return zm, zc
