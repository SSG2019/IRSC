...
def main_train():
    ...
    if not os.path.exists('checkpoints/interference/csirt'):
        os.makedirs('checkpoints/interference/csirt')
    with open('checkpoints/interference/csirt' + '/model_{}.pth'.format(str(I).zfill(2)),
              'wb') as f:
...
def main_test():
    ...
    model = Transceiver().to(device)
    model.load_state_dict(torch.load('./checkpoints/interference/csirt/model_199.pth'))
    ACC1 = []
    ACC2 = []
    for snr in SNR:
        accuracy1 = 0
